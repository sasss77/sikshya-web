...
export default function AdminStudentsPage() {
  ...
  const fetchUsers = useCallback(async (p: number, search: string) => {
    setLoading(true);
    const res = await getAdminUsersAction({ page: p, limit: 10, search, role: "student" });
    ...
    <div style={S.container}>
        ...
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            <div style={S.modalHeader}>
              <h2 style={S.modalTitle}>{editingUser ? "Edit Student" : "Create New Student"}</h2>
      ...
      {isDeleteModalOpen && (
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            ...
            <div style={S.modalBody}>
              <p style={{ margin: 0, color: "#475569" }}>Are you sure you want to delete this student? This action cannot be undone.</p>
            </div>
            <div style={S.modalFooter}>
              <button style={S.btnGhost} onClick={() => setIsDeleteModalOpen(false)}>Cancel</button>
              <button style={S.btnDanger} onClick={confirmDelete}>Yes, Delete Student</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isViewModalOpen && viewingUserDetails && (
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            <div style={S.modalHeader}>
              <h2 style={S.modalTitle}>Student Details: {viewingUserDetails.fullName}</h2>
              <button
              ...
              <div style={{ display: "grid", gap: "16px" }}>
                ...
                {viewingUserDetails.details ? (
                  <>
                    ...
                    <h3 style={{ margin: "0 0 12px 0", fontSize: "16px", color: "#0f172a" }}>Academic Stats</h3>
                    <div>
                      <strong>Institution:</strong> {viewingUserDetails.details.profile?.institution || "N/A"}
                    </div>
                    <div>
                      <strong>Grade Level:</strong> {viewingUserDetails.details.profile?.gradeLevel || "N/A"}
                    </div>
                    <div>
                      <strong>Enrolled Courses:</strong> {viewingUserDetails.details.enrolledCoursesCount}
                    </div>
                    <div>
                      <strong>Appointed Teachers:</strong> {viewingUserDetails.details.appointedTeachersCount}
                    </div>
                    <div>
                      <strong>Total Sessions Attended:</strong> {viewingUserDetails.details.totalSessionsAttended}
                    </div>
                  </>
                ) : (
                  <p style={{ color: "#64748b", marginTop: "16px" }}>No specific student profile details available.</p>
                )}
              </div>
            </div>
            <div style={S.modalFooter}>
